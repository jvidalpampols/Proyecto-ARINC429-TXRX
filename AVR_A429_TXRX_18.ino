...
/*
  ...
  15º.Introduce auto-scroll TX_Buffer[0] for testing dynamic data changes.  Setup_TX_Buffer()and Load_DEI_TX_Buffer() into RX_Display_A429_HEX_Show_Data();
  Data changes  A429_TX_Buffer[0]. A429_Data ++;into routine: ISR_Int_RX1()
  16º. Still problems to keep same label on RX display.
  17º. Introduce auto-scroll TX_Buffer[0] for testing dynamic data changes.  Setup_TX_Buffer()and Load_DEI_TX_Buffer() and Data changes  A429_TX_Buffer[0]. A429_Data ++;
  into RX_Display_A429_HEX_Show_Data(); Removed from routine: ISR_Int_RX1() to gain program stability.
  18º. At Power Up Transmit only Label 001 and 377 with Wrap-around disabled.
  Test Function:
    Enable Wrap-Around and showit by lighting BAT/TEST LED
    To disable Labels,use program refresh time to zero.
  19º.Label 211 decoded as ENG
  AVR_TXRX_A429_16
  20º.Set ENG Menu option.
  21º. RX refresh time in HEX Screen.
  22º. Used same LED for BAT/TEST
  22º. Misc have been replaced by SW_Reset --> makes a reset to DEI1016
  AVR_TXRX_A429_15
  23º. ISR_RX1_Times changed from ISR_RX1_Times >= 10 to ISR_RX1_Times >= 20 So now do not miss values and fixed the RX1 Hung Up // Flag for Auto Testing TX/RX
  24º. Improved First screen Unit S/N. Reset Screen Software Reset  IC DEI1016.
  25º. Help Screens OK
  AVR_TXRX_A429_16
   26º. Warning to disconnect hardware when activate "Test" Wrap-Around
   27º.Refresh time removed from HEX screen,now into BIN screen.
   28º. Changed routines for RX,TX Show BIN. Improved because now is evaluated and printed bit to bit
   AVR_TXRX_A429_17
   29º.RX Buffer increased to 15. Note should be multiple of 3 so as there is 3 labels per page,otherwise routines show data should be changed
   30º Label"000" allowed.
   31º Note program will disregard bit b7 on label so on "ASCII_Octal_to_Octal_Label",after de 3º shift b7 will disapperar. As a result a label 456 will be
   taken as 056.
   AVR_TXRX_A429_18.
   On Get_RX1_Data() >> Deleted:
   32º.  //Select_Word_2_Read(); //SEL HI WORD_2, DI22
   sbi (PORTA, 0); //SEL WORD2 >> PA0 "HI" // //PA0 = HIGH >> Set WORD_2.Pin: DI22
   33º. On Get_RX1_Data() delete label from RX Buffer when Transmittion interval is greater than 1500 miliseconds
